...
impl GoldenRules {
    pub fn check(content: &str, path: &str, lang: &Language) -> Vec<Violation> {
        ...
        match lang {
            ...
            Language::TypeScript | Language::TypeScriptReact => Self::check_typescript(content, path, &mut violations),
    ...
    fn check_rust(content: &str, violations: &mut Vec<Violation>) {
        ...
        if let Some(ref re) = marker_re {
            if re.is_match(content) {
                violations.push(Violation {
                    category: ViolationCategory::Golden,
                    message: "Unresolved TODO/FIXME/HACK/XXX markers: address before merging".into(),
                    tool_hint: Some("Either fix the issue or create a tracking ticket and reference it".into()),
                    line: None,
                });
            }
        }
        ...
        if let Ok(re) = regex::Regex::new(r#"\{:\?\}"#) {
            if re.is_match(content) && !content.contains("#[cfg(test)]") {
                violations.push(Violation {
                    category: ViolationCategory::Golden,
                    message: "Debug formatting `{:?}` in non-test code: remove or gate behind cfg(test)".into(),
    ...
    fn check_typescript(content: &str, _path: &str, violations: &mut Vec<Violation>) {
        ...
        if let Some(ref re) = eslint_re {
            if re.is_match(content) && !content.contains("eslint-enable") {
                violations.push(Violation {
                    category: ViolationCategory::Golden,
                    message: "eslint-disable without matching eslint-enable: disable at minimum scope".into(),
                    tool_hint: Some("Add `// eslint-enable <rule>` after the suppressed block".into()),
                    line: None,
                });
            }
        }
    ...
    fn check_python(content: &str, violations: &mut Vec<Violation>) {
        ...
        if content.contains("from ") && content.contains(" import *") {
            violations.push(Violation {
                ...
                tool_hint: Some("Replace `from module import *` with `from module import Foo`".into()),
                line: None,
            });
        }
    }
    ...
    fn check_go(content: &str, violations: &mut Vec<Violation>) {
            if content.contains("import _ ") {
                violations.push(Violation {
                    category: ViolationCategory::Golden,
                    message: "Blank import `import _` used: ensure it has a side-effect init function".into(),
    ...
    fn check_cross_language(content: &str, path: &str, violations: &mut Vec<Violation>) {
        ...
        let secret_re = regex::Regex::new(r#"(?i)\b(password|secret|api[_\s]?key|token|credential)\b"#).ok();
...
mod tests {
    ...
    fn test_rust_unsafe_without_safety() {
        ...
        assert!(unsafe_v.is_some(), "Should flag unsafe without SAFETY comment: {:?}", violations);
    }
    ...
    fn test_todo_markers_detected() {
        ...
        assert!(todo_v.is_some(), "Should flag TODO markers: {:?}", violations);
    }
    ...
    fn test_console_log_detected() {
        ...
        let log_v = violations.iter().find(|v| v.message.contains("console.log"));
    ...
    fn test_hardcoded_secret() {
        ...
        assert!(secret_v.is_some(), "Should flag hardcoded secret: {:?}", violations);
    }
    ...
    fn test_wildcard_import_python() {
        ...
        assert!(wildcard_v.is_some(), "Should flag wildcard import: {:?}", violations);
    }
    ...
    fn test_loose_equality_javascript() {
        ...
        assert!(eq_v.is_some(), "Should flag loose equality: {:?}", violations);
    }
    ...
    fn test_debug_formatting() {
        ...
        let debug_v = violations.iter().find(|v| v.message.contains("Debug formatting"));
        assert!(debug_v.is_some(), "Should flag debug formatting: {:?}", violations);
    }
    ...
    fn test_eslint_disable_without_enable() {
        ...
        let violations = GoldenRules::check(content, "app.ts", &Language::TypeScript);
        let eslint_v = violations.iter().find(|v| v.message.contains("eslint-disable"));
        assert!(eslint_v.is_some(), "Should flag eslint-disable without enable: {:?}", violations);
    }
